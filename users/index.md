...
All users authenticate with email and password. Two-factor authentication (2FA) via TOTP is available and can be **enforced** for Super Admin and Admin roles from [Settings → Security →](/docs/settings/security).

GodMode uses **JWT-based authentication** with role-based permissions enforced server-side on every API request.

## Related pages

- [Invite a user →](/docs/users/invite-user)
- [Roles and permissions →](/docs/users/roles-permissions)
- [Manage users →](/docs/users/manage-users)
